...
- 在发送栏 **“…” 弹出菜单**中加入 **“BetterTTS-角色” / “BetterTTS 设置”** 入口：弹窗为不同角色指定**说话人（音色）与语言**。
...
| 发送栏“…”菜单“BetterTTS-角色”弹窗 | ✅ 为不同角色指定说话人/语言；含试听按钮 |
...
4. 打开浏览器控制台（F12）应能看到 `[BetterTTS] 模块已加载` 日志；扩展面板出现「🔊 BetterTTS 设置」折叠项、发送栏 “…” 菜单出现 `BetterTTS-角色` / `BetterTTS 设置` 即安装成功。
...
> 若个别 ST 版本没有可注入的 “…” 菜单，扩展会自动退回为聊天输入栏底部的小按钮（不会丢失入口）。
...
打开方式：扩展面板中本扩展的设置折叠项，或发送栏 “…” → “BetterTTS 设置”，或 `/bettertts-settings`。

- **基本设置**：启用 / 流式播放 / 按段朗读 / 朗读旁白 / 语速 / 音量。
- **TTS 服务商**：选择服务商 → 切换对应参数；`获取音色列表`。
- **角色映射**：全局默认音色/语言（也可在 BetterTTS-角色 中配置）。
- **提示词**：编辑框默认**自动填充内置（系统）提示词内容**，可直接查看与修改（空内容时也会再次自动填充）；提供恢复默认/复制/导出(.txt)/导入(.txt|.json)；
...
- **完全没有任何 BetterTTS 入口（扩展面板/发送栏“…”菜单都没有）**：先在浏览器控制台看有没有 `[BetterTTS]` 日志。
