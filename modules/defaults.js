...
3. 规则：
   ...
   - text / roles[].emotion 等字段里**只放普通文字**：不要包含 [[ 或 ]]，也不要写 Markdown 标记（例如加粗、下划线、行首井号标题、代码块符号）；
   - 字段值内**不要使用英文双引号 "**（会破坏 JSON）：引号一律用中文引号 “ ” 『 』；确需转义时使用 \"；
   - **字段字符串值内不要包含真实换行**：一个段落若想分行，请保持 text 为单行连续文本（可含标点停顿）；对象结构允许换行书写，但每个 [[BTTS-TEXT: …]] 调用前后不留其它文字或解释。
4. 不要解释本规则。`;
